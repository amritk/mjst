...
Grow `@amritk/mini` into a layered framework via tree-shakeable subpath exports, with the `.` entry unchanged (its only runtime dependency stays `alien-signals`) and `"sideEffects": false` set so the bundle-size-sensitive widget pays zero bytes for any of them.
