...
Underneath are **two** generator engines, which shipped as `@amritk/generate-parsers` and `@amritk/generate-validators` until this package absorbed them:

- `src/parsers/` — the parser and type engine. Key entry point `src/parsers/generators/build-schema.ts`, reached through the `#parsers/*` subpath import. A parser fuses building the output value with checking it, which is what makes it fast and what stops it reporting.
- `src/validators/` — the validator, coercer and repairer engine. Key entry point `src/validators/generators/build-schema.ts`, reached through `#validators/*`. It keeps the passes apart, so it can say what was wrong. `repair` reads the same fallback table as the parser engine, hoisted into `@amritk/helpers` as `get-default-value` so the two cannot land on different repaired documents.

Both are kept because neither subsumes the other. `generate` composes them into one output directory, which works because both derive the TypeScript type from the same `@amritk/helpers/generate-type-definition` — so the type is declared once and every emitted function talks about it. A single-mode build is byte-identical to the engine that owns that mode, pinned per mode by `src/emits-what-it-composes.test.ts`.

- **Depends on:** `@amritk/helpers`, `json-schema-typed`
- **Subpath imports:** `#internal/*` → `./src/*.ts`, `#parsers/*` → `./src/parsers/*.ts`, `#validators/*` → `./src/validators/*.ts`
- **Public API:** `generate`, `ALL_MODES`, `GeneratedFile`, `GenerateOptions`, `Mode`, `ImportExtension` — the `.` entry and nothing else. The engines are internal; there is no supported way to reach one directly from outside the package.
- **Retired predecessors:** `packages/generate-parsers` and `packages/generate-validators` are private, source-free directories kept only to carry their published CHANGELOGs and a deprecation notice.
...
- **Lossy constructs:** types JSON Schema cannot express are preserved as an `x-mjst` vendor extension rather than dropped. `@amritk/helpers/mjst-extension` defines the shared contract (`MJST_EXTENSION_KEY`, `MjstExtension`, and the readers `getMjstInstanceOf` / `getMjstPrimitive` / `getMjstBrand`), which the type generator, parsers, and validators read to emit the right TypeScript type and runtime checks. The extension is contextual — what a key means depends on the node it sits on — and `@amritk/generate-markdown` reads its documentation keys from the same object (`hidden`, and everything under `markdown`), which the readers here ignore. For the generators it currently carries: `instanceOf` (a runtime class such as `Date`, checked with `instanceof`), `primitive` (a non-JSON primitive such as `bigint`, checked with `typeof`), and `brand` (a type-level nominal brand — the value still validates as its base JSON type at runtime, but the generated TypeScript type is intersected with a unique brand). Brands cannot be auto-detected from the source libraries (Zod/Valibot/Effect brands are type-level or stripped during conversion), so they are opt-in via a hand-authored `x-mjst.brand` keyword — which TypeBox passes through from `Type.String({ 'x-mjst': { brand: 'UserId' } })`.
...
- **Depends on:** `@amritk/helpers`, plus `@amritk/adapters` through its `avro-to-json-schema` subpath alone (so none of that package's optional peers come along). Does no I/O — parsing and cross-file `$ref` resolution stay the caller's job, so the package's inputs are plain values.
...
- **Dialect normalization:** the AsyncAPI default dialect (a draft-07 superset) and declared draft-07 run through `upgradeDraft07Schema`; OpenAPI-format payloads get `nullable` folded; 2020-12 passes through. **Avro is converted** rather than skipped, by the adapter above — `extractAsyncApi(document, { avroEncoding })` picks between the decoded object (`'json'`, the default) and the spec's wire encoding (`'avro-json'`). A format nothing here reads (Protobuf, RAML), or an Avro schema the converter rejects, skips that one schema with an issue — problems are collected, never thrown, except for "not an AsyncAPI document at all".
...
- **Channel contracts key on the wire tag.** `buildChannelContract` projects a channel onto `defineMessages`, keyed by the value each payload pins its discriminator to (`type: { const: 'bot_added' }` → key `bot_added`) and falling back to the AsyncAPI message name only when a payload pins nothing — the name is a document-authoring handle, and 2.x messages inside a `oneOf` often have none. Two messages pinning one tag in one direction collide: first wins, second reported. Payloads only: headers get their own generated tree but have no slot in a contract, since `@amritk/api` contracts describe WebSocket frames.
