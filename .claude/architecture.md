...
- **Design notes:** a single recursive walker (`src/interpreter/interpret.ts`) evaluates the schema against the value; the error array is allocated lazily so valid input never allocates, and the guard path short-circuits on first failure. The only reusable work — compiling `pattern` regexes and resolving local `$ref`s — is memoized per validator. Recursion via `$ref` terminates naturally as the data shrinks. Parity with Ajv is enforced by `src/differential.test.ts` (~132k random/mutated values). OpenAPI `nullable: true` is honored (null accepted regardless of type).
