...
| --- | --- |
| `title`, `description` | The page title, and each property's prose (full markdown, not just the first paragraph) |
...
| `examples` | The derived code example (the first one), plus **Examples:** for the rest |
...
| A recursive `$ref` | Collapsed where it repeats, so generation terminates. The truncation carries the definition's documentation the way any other ref site does — its prose, its `x-doc` type, layout, examples, whether it is hidden — plus what it requires, so an alternative beside it keeps its **Required** markers. Two members are not carried: `x-doc.page` and `x-doc.section`, because a truncation has no shape to move and would arrive there as a bare heading. `$ref: '#'` carries nothing at all — the root is not a definition, and its `x-doc` is the page's own configuration. Reading what a definition requires refuses to re-enter one already on the path, is worked out once per pointer, and stops with an error past 512 levels of composition or 10,000 nodes |
