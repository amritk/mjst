...
3. **All documentation content belongs in the schema.** Prose, code samples,
   ...
   `layout` (`'headings' | 'table' | 'none'`) is read on a property, where it
   lays out that property's children, and on a root `x-doc.sections` entry,
...
9. **A property table's shape is the root `x-doc.table`'s, not the renderer's.**
   ...
   row) the values are listed. An empty `x-doc.type` counts as unset, so it
