...
describe('read-doc-config', () => {
  ...
  it('reads the defaults the schema declares', () => {
    const config = readDocConfig({
      'x-mjst': {
        markdown: { file: 'configuration.md', language: 'javascript', layout: 'table', sort: 'alphabetical' },
      },
    })
  ...
  it('lets the caller override the schema', () => {
    const schema = { title: 'From schema', 'x-mjst': { markdown: { file: 'a.md', language: 'javascript' } } }
  ...
  it('prefers a declared index title over the x-mjst title', () => {
    const config = readDocConfig({
      'x-mjst': { markdown: { title: 'From x-mjst', pages: [{ id: 'index', file: 'a.md', title: 'From the page' }] } },
    })
    expect(config.pages[0]?.title).toBe('From the page')
  })

  it('skips a page that cannot be written or referenced', () => {
    const config = readDocConfig({ 'x-mjst': { markdown: { pages: [{ id: 'a' }, { file: 'b.md' }, 'nope'] } } })
  ...
  it('skips a section with no id to reference it by', () => {
    expect(readDocConfig({ 'x-mjst': { markdown: { sections: [{ title: 'Nameless' }] } } }).sections).toEqual([])
  })

  // The schema is parsed JSON, not validated input.
  it('ignores malformed members and falls back to the defaults', () => {
    const config = readDocConfig({
      'x-mjst': { markdown: { language: 5, layout: 'grid', sort: 7, pages: 'nope', sections: 3 } },
    })
    expect(config.language).toBe('json')
    expect(config.layout).toBe('headings')
    expect(config.pages).toHaveLength(1)
  ...
  it('lets the caller beat a declared index page on title and file', () => {
    const config = readDocConfig(
      { 'x-mjst': { markdown: { pages: [{ id: 'index', file: 'from-schema.md', title: 'From schema' }] } } },
  ...
  it('lets the caller override one member of the table layout', () => {
    const config = readDocConfig(
      { 'x-mjst': { markdown: { table: { requiredFirst: true } } } },
      { table: { type: 'never' } },
    )
    expect(config.table).toEqual({
      type: 'never',
  ...
  it('lets the caller override the required suffix', () => {
    const config = readDocConfig(
      { 'x-mjst': { markdown: { table: { required: '*' } } } },
  ...
  it('keeps an empty required suffix', () => {
    expect(readDocConfig({ 'x-mjst': { markdown: { table: { required: '' } } } }).table.required).toBe('')
  })
  ...
  it('ignores a table member it does not understand', () => {
    const config = readDocConfig({
      'x-mjst': { markdown: { table: { type: 'sometimes', required: 7, requiredFirst: 'yes' } } },
    })
    expect(config.table).toEqual({
      type: 'auto',
      default: 'auto',
