...
describe('build-pages', () => {
  ...
  it('follows a page assignment down into a deeper property', () => {
    const schema = {
      'x-mjst': { markdown: { pages: [{ id: 'deep', file: 'deep.md' }] } },
      properties: {
        a: {
          type: 'object',
          properties: {
            b: { type: 'object', properties: { c: { type: 'string', 'x-mjst': { markdown: { page: 'deep' } } } } },
          },
        },
      },
    }
  ...
  it('groups properties into the sections they name', () => {
    const schema = {
      ...
      properties: {
        a: { type: 'string', 'x-mjst': { markdown: { section: 'two' } } },
        b: { type: 'string', 'x-mjst': { markdown: { section: 'one' } } },
        c: { type: 'string' },
      },
    }
    expect(pageNames(schema)).toEqual([
  ...
  it('keeps a section that no property joins', () => {
    const schema = { 'x-mjst': { markdown: { sections: [{ id: 'intro', title: 'Intro' }] } }, properties: {} }
    expect(pageNames(schema)[0]?.sections).toEqual([{ id: 'intro', entries: [] }])
  })

  it('pulls a nested property up into the section it names', () => {
    const schema = {
      'x-mjst': { markdown: { sections: [{ id: 'emitter', title: 'Emitter' }] } },
      properties: {
        target: {
          type: 'object',
          properties: { options: { type: 'object', 'x-mjst': { markdown: { section: 'emitter' } } } },
        },
      },
    }
    expect(pageNames(schema)).toEqual([
  ...
  it('leaves hidden properties out of every page', () => {
    const schema = { properties: { a: { type: 'string', 'x-mjst': { hidden: true } }, b: { type: 'string' } } }
    expect(pageNames(schema)[0]?.entries).toEqual(['b'])
  })

  it('sorts a section by its own rule, not the page default', () => {
    const schema = {
      'x-mjst': { markdown: { sort: 'schema', sections: [{ id: 'sorted', title: 'Sorted', sort: 'alphabetical' }] } },
      properties: {
        b: { type: 'string', 'x-mjst': { markdown: { section: 'sorted' } } },
        a: { type: 'string', 'x-mjst': { markdown: { section: 'sorted' } } },
      },
    }
  ...
  it('refuses a property assigned to an undeclared page', () => {
    const schema = { properties: { a: { type: 'string', 'x-mjst': { markdown: { page: 'nope' } } } } }
    expect(() => build(schema)).toThrow(/page "nope"/)
  })

  it('refuses a property assigned to an undeclared section', () => {
    const schema = { properties: { a: { type: 'string', 'x-mjst': { markdown: { section: 'nope' } } } } }
    expect(() => build(schema)).toThrow(/section "nope"/)
  })

  it('refuses a section that renders on an undeclared page', () => {
    const schema = { 'x-mjst': { markdown: { sections: [{ id: 'one', page: 'nope' }] } }, properties: {} }
    expect(() => build(schema)).toThrow(/page "nope"/)
  })

  it('refuses a property whose page and section disagree', () => {
    const schema = {
      'x-mjst': { markdown: { pages: [{ id: 'ts', file: 'ts.md' }], sections: [{ id: 'one', title: 'One' }] } },
      properties: { a: { type: 'string', 'x-mjst': { markdown: { page: 'ts', section: 'one' } } } },
    }
