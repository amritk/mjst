...
export const parseCliArgs = (args: readonly string[]): Partial<CliConfig> => {
  ...
  const config: { schema?: string; outDir?: string; typesOnly?: boolean; build?: boolean; logUnmatched?: boolean } = {}
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg.startsWith('--') && arg.includes('=')) {
      ...
      } else if (key === 'log-unmatched') {
        config.logUnmatched = value !== 'false'
      }
    ...
    if (arg === '--schema') {
      ...
      config.build = true
    } else if (arg === '--log-unmatched') {
      config.logUnmatched = true
    }
  }
