...
- de09d82: - Fix line folding in plain, single-quoted, double-quoted and flow scalars stripping Unicode spaces, such as a no-break space or an ideographic space, from the edges of a line. YAML strips only spaces and tabs, and the text an author wrote now survives.
...
- f938dd7: Assert the alias-expansion budget ceiling directly instead of by wall clock.
  ...
  change _whether_ a bomb throws, only how much it materializes first, so
...
- b3364fd: Cap the alias-expansion budget, so padding a document cannot buy a bigger one.
...
- 7e452e1: Close four ways a document could hang, crash, or mis-report the parser.
...
- b957e36: Three more fixes from the same review pass.
