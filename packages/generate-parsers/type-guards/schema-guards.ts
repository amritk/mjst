...
export const hasType = (
  schema: JSONSchema
): schema is SchemaObject & { type: string } => {
...
export const hasPattern = (
  schema: JSONSchema
): schema is SchemaObject & { pattern: string } => {
...
export const hasFormat = (
  schema: JSONSchema
): schema is SchemaObject & { format: string } => {
...
export const hasDefault = (
  schema: JSONSchema
): schema is SchemaObject & { default: unknown } => {
...
export const hasRequired = (
  schema: JSONSchema
): schema is SchemaObject & { required: readonly string[] } => {
...
export const hasMinLength = (
  schema: JSONSchema
): schema is SchemaObject & { minLength: number } => {
...
export const hasExclusiveMinimum = (
  schema: JSONSchema
): schema is SchemaObject & { exclusiveMinimum: number } => {
  return (
    isSchemaObject(schema) &&
...
export const hasExclusiveMaximum = (
  schema: JSONSchema
): schema is SchemaObject & { exclusiveMaximum: number } => {
  return (
    isSchemaObject(schema) &&
...
export const hasMultipleOf = (
  schema: JSONSchema
): schema is SchemaObject & { multipleOf: number } => {
...
export const hasMinItems = (
  schema: JSONSchema
): schema is SchemaObject & { minItems: number } => {
  return isSchemaObject(schema) && 'minItems' in schema && typeof schema.minItems === 'number'
}

/** Type guard to check if schema has maxItems */
export const hasMaxItems = (
  schema: JSONSchema
): schema is SchemaObject & { maxItems: number } => {
...
export const hasUniqueItems = (
  schema: JSONSchema
): schema is SchemaObject & { uniqueItems: boolean } => {
...
export const hasMinProperties = (
  schema: JSONSchema
): schema is SchemaObject & { minProperties: number } => {
  return (
    isSchemaObject(schema) &&
...
export const hasMaxProperties = (
  schema: JSONSchema
): schema is SchemaObject & { maxProperties: number } => {
  return (
    isSchemaObject(schema) &&
...
export const hasRef = (
  schema: JSONSchema
): schema is SchemaObject & { $ref: string } => {
