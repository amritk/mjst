...
const generateObjectParser = (schema: JSONSchema, typeName: string, useRefImports: boolean): string => {
  ...
  // Spread allOf $ref parsers so their field coercions are applied
  // before the explicit property validations below.
...
const generatePatternPropertiesParser = (
  ...
  if (useRefImports && isSchemaObject(patternSchema) && hasRef(patternSchema)) {
    const refParserName = generateParserName(refToName(patternSchema.$ref))
    patternAssignment = `(result as Record<string, unknown>)[key] = ${refParserName}(value);`
