...
- c1ab994: Fix five defects in the generated types, found by running the generators over
...
SchemaObject } | { content: ContentObject }`: the constraint the schema states,
  ...
  **`$ref` imports no longer drift from the emitted text.** A `$ref` the type
  generator inlines _through_ — an `allOf` member naming a conditional definition,
...
version did not. The holdout is OpenAPI 3.0's Callback Object, whose`additionalProperties`covers every key its adjacent`^x-`pattern does not:
...
- 34c5eaf: Import the types a tuple position references, and compare tuple items
  structurally.
  ...
  A tuple-position ref is imported as a _type_. The parser emitter passes a tuple
