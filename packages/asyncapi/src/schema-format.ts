...
// Avro is not a JSON Schema dialect at all, but it is a schema language the
// monorepo already converts, so it is classified rather than refused. All three
// spellings the spec lists — bare, `+json` and `+yaml` — name the same Avro
// schema; the suffix says how the *schema document* was written, and by the time
// it reaches us the parser has already resolved that difference away.
...
export const classifySchemaFormat = (schemaFormat: unknown): SchemaFormatFamily => {
  ...
  if (AVRO_DIALECT.test(schemaFormat)) return 'avro'
