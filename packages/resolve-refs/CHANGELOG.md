...
- fc7276d: Resolve references faster, and read JSON Pointer array indices strictly.
...
- 34c5eaf: **Behavior change:** a `$ref` whose target could not be read is now _kept_ in
  ...
  guard seeded its taken-name set from the _source_ root's `$defs`, but hoists
  attach to the _resolved_ root's — different documents under a pure-`$ref` root
