...
/**
 * A guard that carries its own denial response. Pairing the check (`guard`)
 * with the {@link ResponseContracts} it can answer (`responses`) is what lets
 * `protectedRoute` *derive* the 401/403 from the guard instead of making every
 * route re-declare it: attaching the bundle merges its `responses` into the
 * route, so the status flows into the handler's reply union, the OpenAPI
 * document, and response validation. Build one with `defineGuard` (or a
 * context-bound `guardFactory<Context>()`); spread its `responses` into a
 * browser-shared `defineContract` with `guardResponses` when the native client
 * needs to see the status too.
 */
export type GuardBundle<GResponses extends ResponseContracts, Context = unknown> = {
  /** The statuses this guard may deny with — merged into the route it guards. */
  readonly responses: GResponses
  /** The check itself: deny with one of `responses`' statuses, or pass with `undefined`. */
  readonly guard: (
    context: ContextGuardInput<Context>,
  ) => RouteReply<GResponses> | Response | undefined | Promise<RouteReply<GResponses> | Response | undefined>
}

/**
 * The variance-free structural shape of any {@link GuardBundle} — the bound
 * `protectedRoute`/`guardResponses` accept, so a concrete
 * `GuardBundle<{ 401 }, AppContext>` matches regardless of its `Context` (the
 * `guard` field's contravariant parameter would otherwise block a plain
 * `GuardBundle<…, never>` bound). The precise type is still recovered by the
 * `infer`-based helpers below.
 */
export type AnyGuardBundle = {
  readonly responses: ResponseContracts
  readonly guard: (...args: never) => unknown
}

/** Distributes a union into an intersection — the machinery behind {@link MergedGuardResponses}. */
...
/**
 * Flattens an intersection of object types into one object — `{ a } & { b }`
 * becomes `{ a; b }`. `protectedRoute` needs this on the merged responses:
 * `RouteReply` maps over `keyof`, and the mapped type does not resolve (so the
 * handler's `status: 200` widens to `number`) when handed an unflattened
 * intersection. Preserves `readonly`, which the reply/response types rely on.
 */
export type Simplify<T> = { readonly [Key in keyof T]: T[Key] }

/**
 * The response contracts a guard bundle declares. Constrained structurally on
 * `responses` rather than `GuardBundle<…, Context>` so the `Context` slot's
 * contravariance never blocks the match.
 */
export type GuardResponsesOf<Bundle> = Bundle extends { readonly responses: infer GResponses extends ResponseContracts }
  ? GResponses
  : never

/**
 * The combined {@link ResponseContracts} a tuple of {@link GuardBundle}s
 * contributes: the intersection of each bundle's `responses`, so
 * `[requireSession (401), requireAdmin (403)]` merges to `{ 401 } & { 403 }`.
 * `protectedRoute` unions this with the route's own declared responses. Kept a
 * direct intersection (no `extends infer` wrapper) so it resolves eagerly —
 * a deferred conditional here would rob the handler's reply of its contextual
 * type and widen `status: 200` to `number`. The `Responses &` at the use site
 * already guarantees the `ResponseContracts` bound.
 */
export type MergedGuardResponses<Guards extends readonly AnyGuardBundle[]> = UnionToIntersection<
  GuardResponsesOf<Guards[number]>
>

/**
 * The app `Context` a tuple of {@link GuardBundle}s share — inferred from their
 * `guard` parameter so `protectedRoute` can type the handler's context from the
 * guards alone (build them with a `guardFactory<AppContext>()` and it flows
 * straight through). Guards over different contexts collapse to their union.
 */
export type GuardContextOf<Guards extends readonly AnyGuardBundle[]> = Guards[number] extends {
  readonly guard: (context: ContextGuardInput<infer Context>) => unknown
}
  ? Context
  : never
