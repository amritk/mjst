...
On Bun 1.3 everything converged because everything was paying the same engine slow path; V8 never had the cliff — the Node columns above are flat, frozen or not — and Bun 1.4 has closed it. The generated code keeps counting keys anyway: every alternative was measured on the engine that had the cliff, and each one — `Object.values(obj)`, `Object.keys({ ...obj })`, branching on `Object.isExtensible(obj)` — traded a large, portable regression on the ordinary mutable path for a win on that one engine.
