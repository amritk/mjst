...
export const parseOauth2ApplicationSecurityObject = (input: unknown): Oauth2ApplicationSecurityObject => {
  if (!isObject(input)) {
    return {} as unknown as Oauth2ApplicationSecurityObject;
  }
  const result = {
    ...input,
    ...
    flow: typeof input?.flow === "string" && ["application"].includes(input?.flow as never) ? input?.flow : (input?.flow !== undefined ? String(input?.flow) : "application"),
  ...
  } as unknown as Oauth2ApplicationSecurityObject;
