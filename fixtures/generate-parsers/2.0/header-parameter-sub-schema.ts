...
export const parseHeaderParameterSubSchemaObject = (input: unknown): HeaderParameterSubSchemaObject => {
  if (!isObject(input)) {
    return {} as unknown as HeaderParameterSubSchemaObject;
  }
  const result = {
    ...input,
    ...
    ...((value => value === undefined ? {} : { in: value })(typeof input?.in === "string" && ["header"].includes(input?.in as never) ? input?.in : (input?.in !== undefined ? String(input?.in) : undefined))),
  ...
  } as unknown as HeaderParameterSubSchemaObject;
