...
export const parsePathParameterSubSchemaObject = (input: unknown): PathParameterSubSchemaObject => {
  if (!isObject(input)) {
    return {} as unknown as PathParameterSubSchemaObject;
  }
  const result = {
    ...input,
    required: typeof input?.required === "boolean" && [true].includes(input?.required as never) ? input?.required : (input?.required !== undefined ? Boolean(input?.required) : true),
    ...((value => value === undefined ? {} : { in: value })(typeof input?.in === "string" && ["path"].includes(input?.in as never) ? input?.in : (input?.in !== undefined ? String(input?.in) : undefined))),
  ...
  } as unknown as PathParameterSubSchemaObject;
