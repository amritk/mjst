...
export const parseQueryParameterSubSchemaObject = (input: unknown): QueryParameterSubSchemaObject => {
  if (!isObject(input)) {
    return {} as unknown as QueryParameterSubSchemaObject;
  }
  const result = {
    ...input,
    ...
    ...((value => value === undefined ? {} : { in: value })(typeof input?.in === "string" && ["query"].includes(input?.in as never) ? input?.in : (input?.in !== undefined ? String(input?.in) : undefined))),
    ...
    ...((value => value === undefined ? {} : { type: value })(typeof input?.type === "string" && ["string","number","boolean","integer","array"].includes(input?.type as never) ? input?.type : (input?.type !== undefined ? String(input?.type) : undefined))),
  ...
  } as unknown as QueryParameterSubSchemaObject;
