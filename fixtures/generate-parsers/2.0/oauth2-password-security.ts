...
export const parseOauth2PasswordSecurityObject = (input: unknown): Oauth2PasswordSecurityObject => {
  if (!isObject(input)) {
    return {} as unknown as Oauth2PasswordSecurityObject;
  }
  const result = {
    ...input,
    ...
    flow: typeof input?.flow === "string" && ["password"].includes(input?.flow as never) ? input?.flow : (input?.flow !== undefined ? String(input?.flow) : "password"),
  ...
  } as unknown as Oauth2PasswordSecurityObject;
