...
export const parseOauth2ImplicitSecurityObject = (input: unknown): Oauth2ImplicitSecurityObject => {
  if (!isObject(input)) {
    return {} as unknown as Oauth2ImplicitSecurityObject;
  }
  const result = {
    ...input,
    ...
    flow: typeof input?.flow === "string" && ["implicit"].includes(input?.flow as never) ? input?.flow : (input?.flow !== undefined ? String(input?.flow) : "implicit"),
  ...
  } as unknown as Oauth2ImplicitSecurityObject;
