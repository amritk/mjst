...
export const parseCookieParameterObject = (input: unknown): CookieParameterObject => {
  if (!isObject(input)) return {} as CookieParameterObject;
  return {
    ...input,
    ...(input.in !== undefined && { in: ["cookie"].includes(input?.in as never) ? input?.in : "cookie" }),
    ...(input.style !== undefined && { style: ["form"].includes(input?.style as never) ? input?.style : "form" }),
  } as unknown as CookieParameterObject;
}