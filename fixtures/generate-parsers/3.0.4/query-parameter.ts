...
export const parseQueryParameterObject = (input: unknown): QueryParameterObject => {
  if (!isObject(input)) return {} as QueryParameterObject;
  return {
    ...input,
    ...(input.in !== undefined && { in: ["query"].includes(input?.in as never) ? input?.in : "query" }),
    ...(input.style !== undefined && { style: ["form","spaceDelimited","pipeDelimited","deepObject"].includes(input?.style as never) ? input?.style : "form" }),
  } as unknown as QueryParameterObject;
}