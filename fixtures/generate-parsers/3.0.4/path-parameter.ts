...
export const parsePathParameterObject = (input: unknown): PathParameterObject => {
  if (!isObject(input)) return {
        required: true,
      };
  return {
    ...input,
    ...(input.in !== undefined && { in: ["path"].includes(input?.in as never) ? input?.in : "path" }),
    ...(input.style !== undefined && { style: ["matrix","label","simple"].includes(input?.style as never) ? input?.style : "simple" }),
    required: [true].includes(input?.required as never) ? input?.required : true,
  } as unknown as PathParameterObject;
}