import { type CookieParameterObject, parseCookieParameterObject } from './cookie-parameter';
...
import { type HeaderParameterObject, parseHeaderParameterObject } from './header-parameter';
import { type MediaTypeObject, parseMediaTypeObject } from './media-type';
import { type PathParameterObject, parsePathParameterObject } from './path-parameter';
import { type QueryParameterObject, parseQueryParameterObject } from './query-parameter';
...
export const parseParameterObject = (input: unknown): ParameterObject => {
  ...
  return {
    ...
    in: typeof input?.in === "string" ? input?.in : (input?.in !== undefined ? String(input?.in) : ""),
    ...
    ...(input.style !== undefined && { style: typeof input?.style === "string" ? input?.style : String(input?.style) }),
