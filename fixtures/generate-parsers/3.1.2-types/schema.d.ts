...
type NumericObject = SharedProperties & NumericKeywords & {
    type: 'number' | 'integer';
    format?: NumericFormat;
} & Extensions;
type StringObject = SharedProperties & StringKeywords & {
    type: 'string';
    format?: StringFormat;
} & Extensions;
type ArrayObject = SharedProperties & ArrayKeywords & {
    type: 'array';
} & Extensions;
type ObjectObject = SharedProperties & ObjectKeywords & {
    type: 'object';
} & Extensions;
export type MultiTypeObject = SharedProperties & NumericKeywords & StringKeywords & ArrayKeywords & ObjectKeywords & {
    type?: PrimitiveSchemaType | PrimitiveSchemaType[];
    format?: StringFormat | NumericFormat;
} & Extensions;
export type SchemaObject = UntypedObject | OtherTypes | NumericObject | StringObject | ObjectObject | ArrayObject | MultiTypeObject;
export {};
