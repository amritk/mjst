...
export const parseDocument = (input: unknown): Document => {
  ...
  return {
    ...input,
    openapi: typeof input?.openapi === "string" && /^3\.2\.\d+(-.+)?$/.test(input?.openapi) ? input?.openapi : (input?.openapi !== undefined ? String(input?.openapi) : "1.0.0"),
    ...(input.$self !== undefined && { $self: typeof input?.$self === "string" && /^[^#]*$/.test(input?.$self) ? input?.$self : String(input?.$self) }),
